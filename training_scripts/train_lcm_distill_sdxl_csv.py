        ...
        try:
            if "json" in x:
                ...
                filter_size = (x_json.get(WDS_JSON_WIDTH, 0.0) or 0.0) >= self.min_size and x_json.get(
                    WDS_JSON_HEIGHT, 0) >= self.min_size
...
class SDXLText2ImageCSVDataset(Dataset):
    """CSV 메타데이터와 이미지 폴더를 사용하는 SDXL 학습용 Dataset

    Args:
        csv_path: 메타데이터 CSV 파일 경로
        images_dir: 이미지가 저장된 디렉토리
        resolution: 이미지 해상도
        interpolation_type: 이미지 리샘플링 방법
        use_fix_crop_and_size: 고정 크롭 사용 여부
    """
    ...
    def __init__(
        ...
        self.use_fix_crop_and_size = use_fix_crop_and_size
        self.interpolation_mode = resolve_interpolation_mode(interpolation_type)
        ...
        if 'file_name' in df.columns:
            ...
            raise ValueError("CSV에 file_name, filename, 또는 image_path 컬럼이 필요합니다")
        ...
        if 'text' in df.columns:
            ...
            raise ValueError("CSV에 text 또는 caption 컬럼이 필요합니다")

        # numpy 배열로 변환 (pandas 오버헤드 제거)
        self.filenames = df[filename_col].values
        self.captions = df[caption_col].values

        # 이미지 파일 존재 여부 검증
        self._validate_images()

    def _validate_images(self):
        """이미지 파일 존재 여부를 검증하고 유효한 샘플만 유지 (벡터화)"""
        # 벡터화된 존재 여부 체크
        exists_mask = np.array([(self.images_dir / fn).exists() for fn in self.filenames])

        missing_count = (~exists_mask).sum()

        if missing_count > 0:
            # 처음 10개 누락 파일 로그
            missing_files = self.filenames[~exists_mask][:10]
            for fn in missing_files:
                logger.warning(f"이미지 누락: {self.images_dir / fn}")
            logger.warning(f"총 {missing_count}개 이미지 누락됨")

        # 유효한 샘플만 유지
        self.filenames = self.filenames[exists_mask]
        self.captions = self.captions[exists_mask]

        logger.info(f"유효 샘플: {len(self.filenames)}")

    def __len__(self) -> int:
        return len(self.filenames)

    def __getitem__(self, idx: int):
        """Dataset 샘플 반환 (성능 최적화 버전 - numpy 기반)"""
        # numpy 배열에서 직접 접근 (pandas 오버헤드 없음)
        ...
        if idx % 50 == 0:  # 50개마다 로그
            logger.info(f"🖼️ 이미지 로딩 중: idx={idx}, file={image_filename}")
        ...
        try:
            ...
            # 원본 크기 저장
            orig_size = (self.resolution, self.resolution) if self.use_fix_crop_and_size else (image.width,
                                                                                               image.height)

            # 이미지 변환
            image = TF.resize(image, self.resolution, interpolation=self.interpolation_mode)
            c_top, c_left, _, _ = transforms.RandomCrop.get_params(image,
                                                                   output_size=(self.resolution, self.resolution))
            image = TF.crop(image, c_top, c_left, self.resolution, self.resolution)
            image = TF.to_tensor(image)
            image = TF.normalize(image, [0.5], [0.5])

            crop_coords = (c_top, c_left) if not self.use_fix_crop_and_size else (0, 0)

            # 캡션 유효성 체크
            if not caption or (isinstance(caption, float) and np.isnan(caption)):
                raise ValueError(f"Empty caption for idx={idx}")

            return image, caption, orig_size, crop_coords

        except Exception as e:
            # 재귀 호출 대신 None 반환 (collate_fn에서 필터링)
            logger.warning(f"샘플 로딩 실패 (idx={idx}, file={image_filename}): {e}")
            return None


def collate_fn_filter_none(batch):
    """None 샘플을 필터링하는 collate 함수"""
    batch = [item for item in batch if item is not None]
    if len(batch) == 0:
        # 전체 배치가 실패한 경우 None 반환 (매우 드묾)
        return None
...
def create_csv_dataloader(
    ...
    Args:
        ...
        resolution: 이미지 해상도
        interpolation_type: 보간 방법
        use_fix_crop_and_size: 고정 크롭 사용 여부
        pin_memory: 메모리 고정 여부
        persistent_workers: 지속적 워커 사용 여부

    Returns:
        DataLoader: 설정된 데이터로더
    """
    dataset = SDXLText2ImageCSVDataset(
        csv_path=csv_path,
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=("WebDataset 샤드 경로 또는 URL. CSV 모드에서는 사용되지 않음."),
    )
    # CSV 모드 추가
    parser.add_argument(
        "--use_csv_dataset",
        action="store_true",
        help="WebDataset 대신 CSV 데이터셋 사용",
    )
    parser.add_argument(
        "--csv_path",
        type=str,
        default=None,
        help="CSV 메타데이터 파일 경로 (--use_csv_dataset 사용 시 필수)",
    )
    parser.add_argument(
        "--images_dir",
        type=str,
        default=None,
        help="이미지 디렉토리 경로 (--use_csv_dataset 사용 시 필수)",
    )
    parser.add_argument(
        "--resolution",
        type=int,
    ...
    parser.add_argument(
        ...
        default=8,
        required=False,
        help=("The batch size used when encoding (and decoding) images to latents (and vice versa) using the VAE."
    ...
    # CSV 모드 검증
    if args.use_csv_dataset:
        if not args.csv_path or not args.images_dir:
            raise ValueError("--use_csv_dataset 사용 시 --csv_path와 --images_dir이 필요합니다.")
        if not os.path.exists(args.csv_path):
            raise ValueError(f"CSV 파일을 찾을 수 없습니다: {args.csv_path}")
        if not os.path.exists(args.images_dir):
            raise ValueError(f"이미지 디렉토리를 찾을 수 없습니다: {args.images_dir}")
    else:
        if not args.train_shards_path_or_url:
            raise ValueError("WebDataset 모드에서는 --train_shards_path_or_url이 필요합니다.")
...
def main(args):
    ...
    accelerator = Accelerator(
        ...
        split_batches=
        True,  # It's important to set this to True when using webdataset to get the right number of steps for lr scheduling. If set to False, the number of steps will be divided by the number of processes assuming batches are multiplied by the number of processes
    )
    ...
    tokenizer_one = AutoTokenizer.from_pretrained(args.pretrained_teacher_model,
                                                  subfolder="tokenizer",
                                                  revision=args.teacher_revision,
                                                  use_fast=False)
    tokenizer_two = AutoTokenizer.from_pretrained(args.pretrained_teacher_model,
    ...
    text_encoder_cls_one = import_model_class_from_model_name_or_path(args.pretrained_teacher_model,
                                                                      args.teacher_revision)
    text_encoder_cls_two = import_model_class_from_model_name_or_path(args.pretrained_teacher_model,
                                                                      args.teacher_revision,
                                                                      subfolder="text_encoder_2")

    text_encoder_one = text_encoder_cls_one.from_pretrained(args.pretrained_teacher_model,
                                                            subfolder="text_encoder",
                                                            revision=args.teacher_revision)
    text_encoder_two = text_encoder_cls_two.from_pretrained(args.pretrained_teacher_model,
                                                            subfolder="text_encoder_2",
                                                            revision=args.teacher_revision)

    # 4. Load VAE from SD-XL checkpoint (or more stable VAE)
    vae_path = (args.pretrained_teacher_model
    ...
    teacher_unet = UNet2DConditionModel.from_pretrained(args.pretrained_teacher_model,
                                                        subfolder="unet",
                                                        revision=args.teacher_revision)
    ...
    time_cond_proj_dim = (teacher_unet.config.time_cond_proj_dim
                          if teacher_unet.config.time_cond_proj_dim is not None else args.unet_time_cond_proj_dim)
    ...
    # Dataset 생성 (CSV 또는 WebDataset)
    if args.use_csv_dataset:
        logger.info("CSV 데이터셋 모드 사용")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        logger.info(f"📌 Epoch {epoch} 시작")
        for step, batch in enumerate(train_dataloader):
            # 🔍 배치 로딩 확인 로그
            if step == 0:
                logger.info(f"✅ 첫 배치 로딩 성공! (Epoch {epoch})")

            # 배치가 None이면 스킵 (전체 배치 로딩 실패 시)
            if batch is None:
                logger.warning(f"Epoch {epoch}, Step {step}: 전체 배치 로딩 실패, 스킵")
                continue

            with accelerator.accumulate(unet):
                # 1. Load and process the image, text, and micro-conditioning (original image size, crop coordinates)
                logger.info(f"🔄 Step {step}: 배치 언패킹 시작...")
                image, text, orig_size, crop_coords = batch

                logger.info(f"🔄 Step {step}: 이미지 GPU 전송 중...")
                image = image.to(accelerator.device, non_blocking=True)
                logger.info(f"🔄 Step {step}: 텍스트 임베딩 계산 중...")
                encoded_text = compute_embeddings_fn(text, orig_size, crop_coords)

                # VAE dtype은 초기화 시 이미 설정됨
                logger.info(f"🔄 Step {step}: VAE 인코딩 준비 중...")
                if args.pretrained_vae_model_name_or_path is not None:
                ...
                logger.info(f"🔄 Step {step}: VAE 인코딩 시작...")
                latents = []
                ...
                logger.info(f"✅ Step {step}: VAE 인코딩 완료!")
                ...
                logger.info(f"🔄 Step {step}: 타임스텝 샘플링 중...")
                ...
                logger.info(f"🔄 Step {step}: boundary scaling 계산 중...")
                ...
                logger.info(f"🔄 Step {step}: 노이즈 샘플링 및 추가 중...")
                noise = torch.randn_like(latents)
                noisy_model_input = noise_scheduler.add_noise(latents, noise, start_timesteps)

                # 5. Sample a random guidance scale w from U[w_min, w_max] and embed it
                logger.info(f"🔄 Step {step}: guidance scale embedding 계산 중...")
                ...
                logger.info(f"🔄 Step {step}: Student UNet forward pass 시작...")
                noise_pred = unet(
                    noisy_model_input,
                    start_timesteps,
                ...
                logger.info(f"✅ Step {step}: Student UNet 완료!")
                ...
                # solver timestep.
                logger.info(f"🔄 Step {step}: Teacher UNet forward pass 시작...")
                with torch.no_grad():
                    if torch.backends.mps.is_available():
                        autocast_ctx = nullcontext()
                    else:
                    ...
                    with autocast_ctx:
                        ...
                        logger.info(f"🔄 Step {step}: Teacher UNet (conditional) 실행 중...")
                        ...
                        logger.info(f"✅ Step {step}: Teacher UNet (conditional) 완료!")
